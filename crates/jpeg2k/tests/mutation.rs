...
use jpeg2k::error::CodecError;
use jpeg2k::{decode, encode, Jpeg2kOptions};

const W: u32 = 16;
const H: u32 = 16;

/// Deterministic, well-compressing gradient so the base stream stays small
/// (keeps the truncation/bit-flip loops fast).
fn sample_image() -> Vec<u16> {
    (0..H)
        .flat_map(|y| (0..W).map(move |x| ((x + y) & 0xFF) as u16))
        .collect()
}

fn base_stream() -> Vec<u8> {
    let mut buf = Vec::new();
    encode(&sample_image(), W, H, &Jpeg2kOptions::default(), &mut buf).expect("encode base image");
    buf
}

/// A decode result must be `Err`, or an `Ok` of exactly `W*H` samples with the
/// requested dimensions. Anything else is a robustness failure.
...
/// (c) The trailing EOC marker is mandatory: chopping it must yield `Err`.
#[test]
fn chopped_trailing_marker_is_err() {
    ...
    assert_eq!(
        &base[base.len() - 2..],
        &[0xFF, 0xD9],
        "expected the stream to end with the EOC marker (FF D9)"
    );
    let chopped = &base[..base.len() - 2];
    assert!(
        decode(chopped, W, H).is_err(),
        "decode must reject a stream missing its trailing EOC marker"
    );
}
