...
//! Fuzz target: the JPEG 2000 decoder must never panic on arbitrary input.
//!
//! The first 4 bytes are consumed as two little-endian `u16` dimensions, each
//! clamped to `1..=512`, and the remainder is fed to `jpeg2k::decode` at those
//! dimensions. Any `Err` is an acceptable, well-typed rejection; only a panic
//! (unwrap, index-out-of-bounds, unbounded allocation) is a finding.
...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = jpeg2k::decode(&data[4..], width, height);
});
