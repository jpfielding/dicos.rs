...
//! Fuzz target: the JPEG Lossless decoder must never panic on arbitrary input.
//!
//! The first 4 bytes are consumed as two little-endian `u16` dimensions, each
//! clamped to `1..=512`, and the remainder is fed to `jpegli::decode` at those
...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = jpegli::decode(&data[4..], width, height);
});
